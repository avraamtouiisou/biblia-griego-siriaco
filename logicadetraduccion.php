...
$textoGriego = 'Εἰ δέ τις ὑμῶν λείπεται σοφίας, αἰτείτω παρὰ τοῦ διδόντος θεοῦ πᾶσιν ἁπλῶς καὶ μὴ ὀνειδίζοντος καὶ δοθήσεται αὐτῷ';
    $textoGriego = normalizeAccent(mb_strtolower($res['text'], 'UTF-8'));
    ...
    echo '<tx class="d-block text-secondary-emphasis" style="text-transform:uppercase;">'.$res['verse'].' '.$textoGriegoConSigmaArcaica.'</tx>'.PHP_EOL;
    echo '<tb class=d-block><a class=d-inline href="bibloi.php?book='.$booko.'&chapter='.$chapter.'&verse='.$res['verse'].'&resources%5B%5D=tanslg&filtro=">'.$res['verse'].'</a>'./*$res['verse'].*/' <t class=d-inline>'.$textoGriego.'</t></tb>'.PHP_EOL;
    ...
    foreach ($palabras as $id => $palabra) {
        ...
        // el unacc es redundante. podria usar la misma $palabra si tuviera un buen editor en el telefono que no renderice mal las letras con acento. lo mismo con el lex para comparar por ejemplo con los verbos que convierten en porque la palabra oti. para lex deberia ser lexaccented
        $sqld = "SELECT * FROM lxx WHERE gr = '".$palabra."'";
        $stmt = $pdo->query($sqld);
        $usuario = $stmt->fetch(PDO::FETCH_ASSOC);
    
        if ($usuario) {
            $spanishdVar = !empty($usuario['spanishd'])?$usuario['spanishd']:$usuario['spanish'];
            //toLowerCamelCase erased
            $s = ($spanishdVar);//
            //$s = toLowerCamelCase($usuario['spanish']);//-
            //$s = str_replace(' ', '', $usuario['spanish']);//-
             //este codigo debe comentarse despues de que se eliminar cualquier pattern en la traduccion contextual y comprobarse 
            //$s = "<sp $id class='".$palabra."' morp='".$usuario['morp']."'>"/*.$usuario['morp'].' '*/.$s."</sp> ";
            $sArray[] = $s;
            $mArray[] = !empty($usuario['morpd'])?$usuario['morpd']:$usuario['morp'];
            $iArray[] = $id;
            $gArray[] = $palabra;
            $l_uArray[] = array('lex' => $usuario['lex'], 'unacc' => $usuario['unacc']);
            //echo $s;
    ...
    foreach ($sArray as $ids => &$su) {
        ...
        if (isset($mArray[$ids+1]) && $mArray[$ids+1] == 'X'){
            ...
            //en cuanto a la _u cual digo anteriormente que estaria demas con un buen editor y claro todo lo que tenga que ver con ello en adelante
        }
        
        //$su = $sArray[$ids];
        $sFo = $sArray[$ids];
        
        //false genitivo tambien como el siguiente pero para femeninos
        if (false && $mArray[$ids] == 'RA.GSF' && isset($mArray[$ids+1]) && $mArray[$ids+1] == 'N.GSF'){
            $sArray[$ids] = '<s class=d-inline-block>'.$su.'</s> ';
        }
        
        //N.GSN   
        if (false && $mArray[$ids] == 'RA.GSN' && isset($mArray[$ids+1]) && ($mArray[$ids+1] == 'N.GSN' || $mArray[$ids+1] == 'N.GSM')){
            $sArray[$ids] = '<s class=d-inline-block>'.$su.'</s> ';
        }
        
        //este pattern deberia ser util para todos los condicionales hasta que haga la correccion que tengo que hacer
        //$pattern = '/<sp[^>]*>(.*?)<\/sp>/';
        //eliminar de si el anterior esta en genitivo articulo. esto deberia arreglarse en el sentido de que los nombres en genitivo no deberian guardar la palabra de ni el articulo sino directamento el significado de la palabra. el articulo de manera automatica con codigo.
        //Luego agregué otro condicional para eliminar el articulo pero esto no deberia ser asi como dice anteriormente. Ese error deberia solucionarse.
        //echo var_dump($mArray);
        //$pattern = '/<sp[^>]*>(.*?)<\/sp>/';
        //se me ocurrio algo mejor que esto que pongo en desarrollo en el condicional siguiente
        if (false && in_array($mArray[$ids], array('N.GSN', 'N.GSM', 'N.GSF', 'N.DSM'/*hay que tomar en cuenta este anterior morfema porque hay nombres propios cuya forma dativa es la misma forma genitiva*/)) && isset($mArray[$ids-1]) && in_array($mArray[$ids-1], array('RA.GSN', 'RA.GSF', 'RA.GSM'))){
            //($mArray[$ids-1] == 'RA.GSN' || $mArray[$ids-1] == 'RA.GSF')
            //dĒ la
            $new_html_string = str_replace('dĒ', '×', $su);
            
            /*$new_html_string = preg_replace_callback($pattern, function($matches) {
                    // $matches[1] es el contenido dentro de la etiqueta <p>
                    $content = $matches[1];
                    // Realizamos el reemplazo del carácter dentro del contenido
                    $content = str_replace('de', '×', $content);
                    // Devolvemos la etiqueta <p> con el contenido modificado
                    return $content;
                    //return '<p>' . $content . '</p>';
                }, $new_html_string);*/

            //esto se podria resumir si quito la etiqueta sp anteriormente, armando arreglos para todo y las etiquetas lad pongo de ultimo
            $gn = definiteArticle(str_replace(' ', '', strip_tags($su)));
        ...
        //falta nota aqui. no se si antes o despues deberia cambiar el codigo para agregar la palabra de a los genitivos que no esten antecedidos por articulo genitivo. es decir agregar el de si la palabra anterior no es igual a de si la palabra anterior no es un un articulo genitivo
        //una vez se elimine dĒ se deberia eliminar este codigo y convertirlo en todas las veces que se deba agregar como sufijo la palabra Δε como en el caso fe que la palabra anterior no sea un articulo en genitivo para N.GSF. o para adjetivos si antes no hay preposiciones traducibles en de?
        /*if (in_array($mArray[$ids], array('A.GPN')) && $sArray[$ids-1] == 'de'){
            $sFo = str_replace('dĒ', '∞', $su);
        }*/
